...
export async function loader(_: LoaderFunctionArgs) {
  ...
  const staticPaths = ["/", "/products/malibu-professional-spray-1l", "/articles"];
  const urls = [...staticPaths.map((path) => ({ path, updated_at: null })), ...articles.filter((article: any) => !article.noindex).map((article: any) => ({ path: `/articles/${article.slug}`, updated_at: article.updated_at }))];
