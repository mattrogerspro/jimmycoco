...
const logoTag = (data) => `<a href="${data.homeUrl || 'https://www.jimmycoco.pro'}" style="display:inline-block;text-decoration:none;"><img src="${DEFAULT_LOGO_URL}" width="240" height="70" alt="Sunless by Jimmy Coco" style="display:block;width:240px;max-width:100%;height:auto;border:0;outline:none;text-decoration:none;"></a>`;
...
function renderBlock(block, theme) {
  ...
  switch (block.type) {
    ...
    case 'steps':
      return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:2px 0 24px 0;">${(block.items || []).map((item, index) => `<tr><td valign="top" style="width:30px;padding:5px 0;color:${accent};font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;font-weight:700;">${index + 1}.</td><td class="sans" style="padding:5px 0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:15.5px;line-height:1.55;color:${text};">${item}</td></tr>`).join('')}</table>`;
    case 'offer':
