...
test('new standalone trade records default to live while related records inherit their parent mode', () => {
  ...
  assert.match(migration, /new\.data_mode := case when coalesce\(create_as_demo, false\) then 'demo' else 'live' end/)
  assert.match(migration, /tg_table_name = 'resellers' and new\.application_id is not null/)
  assert.match(migration, /tg_table_name = 'reseller_orders'/)
  assert.match(migration, /tg_table_name = 'invoices'/)
  assert.match(migration, /new\.data_mode := inherited_mode/)
})
