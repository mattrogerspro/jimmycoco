...
- **Registry / send:** registered but disabled in `shared/campaign-registry.js` on its legacy delivery path. Migrate and validate its runtime-variable contract separately before switching it to repository-rendered direct HTML; any activation still requires human approval.
