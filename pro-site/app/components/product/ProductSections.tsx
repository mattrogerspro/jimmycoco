...
export function CrossSell({ state, setState }: { state: PurchaseState; setState: Dispatch<SetStateAction<PurchaseState>> }) {
  ...
  return <section className="steps-band cross-sell" id="complete-order"><div className="wrap"><p className="eyebrow">Add your retail products · Step 2</p><h2>Would you like to add any retail?</h2><p className="sub">Open a product to choose its quantity and see its own volume levels, trade price and potential profit.</p><div className="cross-sell-rail"><RetailProductCards orderMode quantities={state.retail} onQuantityChange={setRetailQuantity} renderConfigurator={(id, quantity) => <OrderConfiguratorModal item={id} state={state} setState={setState} triggerLabel={quantity ? "Edit this item" : "Configure this item"} triggerClassName="retail-add retail-configure-button" />} /><span className="cross-sell-scroll-cue" aria-hidden="true"><small>More</small><b>→</b></span></div><div className="cross-sell-next"><div className="cross-sell-next-copy"><p>Each product is configured separately. Happy with the mix?</p><small>*Maximum profit at full retail price.</small></div><div className="cross-sell-next-actions"><a className="btn btn-dark" href="#order">Continue to final order</a></div></div></div></section>;
}
