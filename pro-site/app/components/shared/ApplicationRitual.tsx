...
export function ApplicationRitual() {
  return <section className="ritual" id="ritual">
    ...
    <div className="rit-grid">{cards.map(({ title, background, image, alt, copy }, index) => <div className={`rit-panel${image ? "" : " rit-panel-plain"}`} key={title} style={{ backgroundImage: `url('${background}')` }}>
