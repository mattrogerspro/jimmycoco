...
export function ApplicationRitual() {
  return <section className="ritual" id="ritual">
    ...
    <div className="rit-grid">{cards.map(({ title, background, image, alt, copy }, index) => <div className={`rit-panel${image ? "" : " rit-panel-plain"}`} key={title} style={image ? { backgroundColor: "#BD9375" } : { backgroundImage: `url('${background}')` }}>
      ...
      <div className="rit-copy"><h3>{title}</h3><i aria-hidden="true">{icons[index]}</i><p>{copy}</p><span className="rit-dash" /></div>
    </div>)}</div>
    <div className="rit-metrics"><div><b>35<small>ml</small></b><span>Ideal<br />application</span></div><div><b>6–8<small>hours</small></b><span>Classic<br />development</span></div></div>
  </section>;
}
